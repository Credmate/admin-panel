...
//* interface
import { BorrowerData, UserData, PaymentsData } from "@/interface/interface";
...
const VerifyPayments = () => {
  ...
  const columns: ColumnsType<PaymentsData> = [
    {
      title: (
    ...
    },
    {
      ...
      render: (status) => {
        ...
        return (
          <span
            className={`px-2 py-1 rounded-full text-sm ${getpaymentStyle(
              status
            )}`}
          >
            {status}
        ...
        const actionItems = [
          {
            ...
            label: "View Details",
            onClick: () =>
              router.push(`/dashboard/verifypayments/${record.key}`),
          },
        ];
